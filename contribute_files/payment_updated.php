<?php
// UPDATED for Feature 17 (Order Management): on successful payment we now
// also insert a row into `orders` + `order_items` so order_management.php
// has something to show. Stock-update query also switched to a prepared
// statement. Everything else is unchanged from the original file.
session_start();
...
if (isset($_POST['confirm_payment'])) {
    if (!empty($_SESSION['cart'])) {
        ...
        try {
            ...
            // NEW: record the order (Feature 17 - Order Management)
            $status = "Completed";
